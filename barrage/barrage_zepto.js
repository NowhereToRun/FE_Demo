 ...
 * @Last Modified time: 2017-04-14 17:51:48
 * 
 * 弹幕组件，依赖zepto或jquery
 */
...
; (function (win, doc) {
    ...
    DanMu.prototype.draw = function (isCircle) {
        ...
        while (j < liLength) {
            for (var k = 0; k < this.rows; k++) {
                ...
                item.css({
                    'transform': 'translateX(-' + (this.canvas.width + liWidth) + 'px)',
                    '-webkit-transform': 'translateX(-' + (this.canvas.width + liWidth) + 'px)',
                    '-moz-transform': 'translateX(-' + (this.canvas.width + liWidth) + 'px)',
                    'transition': 'transform ' + (this.showTime) + 'ms linear ' + Math.round(Math.random() * this.randomMax) + 'ms',
                    '-webkit-transition': 'transform ' + (this.showTime) + 'ms linear ' + Math.round(Math.random() * this.randomMax) + 'ms',
    ...
    var Canvas = function (parent, layerClass, rows) {
        ...
        this.layer = $('<ul></ul>', {
            class: layerClass,
            css: { width: this.width, height: this.height, zIndex: 999, overflow:'hidden' }
        })
